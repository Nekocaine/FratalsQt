...
VonKochReverse::VonKochReverse()
{
    name = "Anti-flocon de Von Koch";   //nom
...
void VonKochReverse::gen(QImage &img, const QString &its_str, const QColor &fg, const QColor &bg) const {
    int its = iterations.indexOf(its_str) + 1;
    int size = img.size().height();

    img.fill(bg);//couleur de fond

    double cx1 = qCeil(size*1/5);
    ...
    VonKochBase::itVonKoch(painter, cx2, cy2, cx1, cy1, its);
    VonKochBase::itVonKoch(painter, cx3, cy3, cx2, cy2, its);
    VonKochBase::itVonKoch(painter, cx1, cy1, cx3, cy3, its);
